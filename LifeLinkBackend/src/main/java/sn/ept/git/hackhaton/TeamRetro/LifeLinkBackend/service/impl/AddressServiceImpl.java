...
import sn.ept.git.hackhaton.TeamRetro.LifeLinkBackend.repository.AddressRepository;
import sn.ept.git.hackhaton.TeamRetro.LifeLinkBackend.service.AddressService;
import sn.ept.git.hackhaton.TeamRetro.LifeLinkBackend.service.dto.AddressDTO;
import sn.ept.git.hackhaton.TeamRetro.LifeLinkBackend.service.mapper.AddressMapper;

/**
 * Service Implementation for managing {@link sn.ept.git.hackhaton.TeamRetro.LifeLinkBackend.domain.Address}.
 */
@Service
@Transactional
public class AddressServiceImpl implements AddressService {
    ...
    /**
     *  Get all the addresses where Donor is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<AddressDTO> findAllWhereDonorIsNull() {
        LOG.debug("Request to get all addresses where Donor is null");
        return StreamSupport.stream(addressRepository.findAll().spliterator(), false)
            .filter(address -> address.getDonor() == null)
            .map(addressMapper::toDto)
